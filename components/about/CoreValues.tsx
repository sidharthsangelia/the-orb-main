...
export const CoreValuesSection = ({ coreValues }: CoreValuesSectionProps) => {
  ...
  return (
    <div ref={valuesRef} className="w-full max-w-full overflow-hidden">
        ...
        <div className="grid gap-6 sm:gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {displayValues.map((value, index) => (
            <motion.div
              ...
              className="text-center space-y-4 sm:space-y-6 p-6 sm:p-8 rounded-xl sm:rounded-2xl bg-[#eae4d2]/10 dark:bg-[#0c0d0d]/20 border border-[#575846]/10 hover:bg-[#eae4d2]/20 dark:hover:bg-[#0c0d0d]/30 transition-all duration-300 group w-full"
            >
              {/* Icon */}
              <div className="inline-flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-lg sm:rounded-xl bg-gradient-to-br from-[#487052] to-[#509e8e] text-white group-hover:scale-110 transition-transform duration-300 mx-auto">
                {getIconComponent(value.icon, "h-5 w-5 sm:h-6 sm:w-6")}
              </div>
              
              {/* Title */}
              <h3 className="text-lg sm:text-xl font-semibold text-[#0c0d0d] dark:text-[#eae4d2] leading-tight">
                {value.title}
              </h3>
              
              {/* Description */}
              <p className="text-[#575846] dark:text-[#eae4d2]/70 text-sm sm:text-base leading-relaxed">
                {value.description}
              </p>
            </motion.div>
          ))}
