...
export const PostCard = ({ post, large = false, className = '' }: PostCardProps) => {
        ...
        <div className={`relative overflow-hidden ${large ? 'h-64 md:h-80' : 'h-48'}`}>
          ...
          {post.category && (
            <Badge 
              ...
              style={{ 
                backgroundColor: `${post.category.color?.hex || 'hsl(var(--primary))'}15`,
                color: post.category.color?.hex || 'hsl(var(--primary))',
                borderLeft: `3px solid ${post.category.color?.hex || 'hsl(var(--primary))'}`
              }}
            >
              {post.category.title}
            </Badge>
...
export const PostCardSkeleton = ({ large = false }: { large?: boolean }) => (
