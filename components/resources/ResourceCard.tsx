    ...
    <Card className={`group overflow-hidden hover:shadow-xl transition-all duration-500 bg-card border-border ${large ? 'md:col-span-2 lg:col-span-2' : 'w-full h-full'} ${className}`}>
        ...
        <div className={`relative overflow-hidden ${large ? 'h-64 md:h-80' : 'h-24 sm:h-32'}`}>
          {resource.image ? (
            <Image
              ...
              sizes={large ? "(max-width: 768px) 100vw, 66vw" : "(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"}
            />
          ) : (
            <div className="w-full h-full bg-gradient-to-br from-primary/10 to-secondary/10 flex items-center justify-center">
              <div className="text-center">
                <div className="w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-2 bg-primary/20 rounded-full flex items-center justify-center">
                  <Leaf className="h-4 w-4 sm:h-6 sm:h-6 text-primary" />
                </div>
                <span className="text-primary text-xs sm:text-sm font-medium">Resource</span>
              </div>
            </div>
          )}
      ...
      <CardContent className="p-3 sm:p-6 space-y-2 sm:space-y-3">
        ...
        {resource.description && (
          <p className={`text-muted-foreground mb-2 sm:mb-4 line-clamp-2 sm:line-clamp-3 leading-relaxed ${large ? 'text-sm sm:text-base' : 'text-xs sm:text-sm'}`}>
            {resource.description}
          </p>
        )}
...
export const ResourceCardSkeleton = ({ large = false }: { large?: boolean }) => (
  <Card className={`overflow-hidden bg-card border-border ${large ? 'md:col-span-2 lg:col-span-2' : 'w-full h-full'}`}>
    <div className={`relative bg-muted animate-pulse ${large ? 'h-64 md:h-80' : 'h-24 sm:h-32'}`} />
    <CardContent className="p-3 sm:p-6 space-y-2 sm:space-y-3">
      <div className="flex items-center gap-2">
        <div className="h-4 w-16 bg-muted animate-pulse rounded" />
        <div className="h-3 sm:h-4 w-24 bg-muted animate-pulse rounded" />
      </div>
      <div className={`h-5 sm:h-6 w-full bg-muted animate-pulse rounded ${large ? 'h-7 sm:h-8' : ''}`} />
      <div className={`h-5 sm:h-6 w-3/4 bg-muted animate-pulse rounded ${large ? 'h-7 sm:h-8' : ''}`} />
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="flex items-center gap-2">
          <div className="h-6 w-6 sm:h-8 sm:h-8 bg-muted animate-pulse rounded-full" />
